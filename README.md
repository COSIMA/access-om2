...
The model should be installed to somewhere that has enough disk space for the model inputs and output. On raijin it should be downloaded to `/short/${PROJECT}/${USER}`.

Here's how to do a new install. This will download the input and source files and build and install model configurations at 1, 1/4 and 1/10th degree resolution with JRA55 forcing and also at 1 degree resolution with CORE forcing. It will also set the environment variable `ACCESS_OM_DIR` to `/short/${PROJECT}/${USER}/access-om2`, the path to the installation.
```{bash}
...
./install.sh
```
(This is all you need to do to compile the model, but if you want to do something more complicated, see the old, detailed install instructions see [here](https://github.com/OceansAus/access-om2/wiki/Old,-detailed-install-instructions).)
...
More details are [here](https://github.com/OceansAus/access-om2/wiki/Contributing-to-model-configurations). 
...
You will need write access to `/g/data3/hh5/tmp/cosima/`, or at least to the subdirectory you use. For access, contact the CMS team <climate_help@nci.nf.org.au>; access is controlled by FACLs.
